...
{#snippet Input(name,type,label,placeholder='')}
    ...
    <input {type} id={name} {name} {placeholder} required class="w-full mt-1 mb-4 border border-pg-sky rounded-md focus:border-pg-sky"/>
{/snippet}
...
<form class="mt-10 mx-5 flex-row justify-center" method="POST" action="?/createInventory" use:enhance enctype="multipart/form-data">
    ...
    {@render Input("ownerNunber", "number", "mobile")}
    ...
    {@render Input("pgLocation", "text", "location", "please provide the location link")}
